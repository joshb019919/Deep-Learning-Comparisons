    ...
    for batch_size in [2, 32, 256, 512, 4096, 8192]:
        for iterations in [50, 100, 200, 500, 2000]:
            w, train_losses, test_losses = gradient_descent(
                ...
                batch_size=batch_size, max_iters=iterations,
            ...
            if "figures":
                plot_loss_curves(
                    ...
                    title=f"Linear Regression - Batch Size {batch_size} ({iterations} iterations)",
                    ylabel="MSE",
                    save_path=f"reports/figures/linear_bs_{batch_size}_{iterations}_iterations.png"
                )
            ...
            if "reports":
                ...
                save_metrics_to_csv(
                    ...
                    metrics, 
                    fieldnames=["model", "batch_size", "iterations", "final_train_loss", "final_test_loss"]
                )
    ...
    for batch_size in [2, 32, 256, 512, 4092, 8196]:
        for iterations in [50, 100, 200, 500, 2000]:
            w, train_losses, test_losses = gradient_descent(
                ...
                tol=1e-6, batch_size=batch_size, max_iters=iterations,
                X_val=X_test, y_val=y_test
            )
            ...
            if "figures":
                plot_loss_curves(
                    train_losses, test_losses,
                    title=f"Logistic Regression - Batch Size {batch_size} ({iterations} iterations)",
                    ylabel="Cross Entropy",
                    save_path=f"reports/figures/logistic_bs_{batch_size}_{iterations}_iterations.png"
                )
